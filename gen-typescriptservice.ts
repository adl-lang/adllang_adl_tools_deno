...
import { LoadedAdl, parseAdl } from "./utils/adl.ts";
 ...
 */
function expandTypeAliases(loadedAdl: LoadedAdl, typeExpr: TypeExpr): TypeExpr {
  let typeRef = typeExpr.typeRef;
  let parameters = typeExpr.parameters;
...
export async function genTypescriptService(params: GenTypescriptServiceParams) {
  ...
  // Ignore error due non used imports in the first line
  code.add(
      `// @ts-ignore - Ignore unused imports typescript error`,
  );
